...
class CircuitBreaker {
    ...
    failureCount = 0;
    openUntil = 0;
    constructor(failureThreshold = 5, cooldownMs = 30000, logger) {
    ...
    async execute(operation) {
        ...
        try {
            ...
            if (this.failureCount >= this.failureThreshold) {
                ...
                if (this.logger) {
                    this.logger.error('Circuit breaker opened due to consecutive failures', error instanceof Error ? error : new Error(String(error)), {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
            ...
            if (attempt === maxAttempts - 1) {
                if (logger) {
                    logger.error('Max retry attempts exhausted', error instanceof Error ? error : new Error(String(error)), {
