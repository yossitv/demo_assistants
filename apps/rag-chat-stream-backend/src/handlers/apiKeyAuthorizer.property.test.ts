...
describe('API Key Authorizer - Property Tests', () => {
  ...
  describe('Property 6: API key not logged', () => {
    ...
    const containsApiKeyValue = (calls: any[][], trimmedKey: string): boolean => {
          ...
          if (typeof arg === 'string') {
            try {
              ...
              if (parsed && typeof parsed === 'object') {
                if (Object.values(parsed).some(value => typeof value === 'string' && value.includes(trimmedKey))) {
                  return true;
                }
                continue;
              }
            } catch {
              // ignore JSON parse errors and fall back to string inclusion check
            }

            if (arg.includes(trimmedKey)) {
              return true;
            }
          } else if (typeof arg === 'object' && arg !== null) {
            if (Object.values(arg).some(value => typeof value === 'string' && value.includes(trimmedKey))) {
              return true;
            }
          }
        }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (apiKey) => {
            ...
            return !containsApiKeyValue(allCalls, trimmedKey);
          }
        ),
        { numRuns }
      );
