...
describe('retryWithBackoff', () => {
  ...
  it('respects maxDelayMs cap', async () => {
    ...
    // Second retry: 1500ms (capped from 2000)
    // Third retry: 1500ms (capped from 4000)
    expect(delays[0]).toBeGreaterThanOrEqual(950);
    expect(delays[0]).toBeLessThan(1100);
    expect(delays[1]).toBeGreaterThanOrEqual(1450);
    expect(delays[1]).toBeLessThan(1600);
    expect(delays[2]).toBeGreaterThanOrEqual(1450);
    expect(delays[2]).toBeLessThan(1600);
  });
