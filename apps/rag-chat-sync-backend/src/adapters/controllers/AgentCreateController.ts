...
import { validateAgentCreateBody } from '../../shared/validation';
...
export class AgentCreateController {
  ...
  constructor(
    private readonly useCase: CreateAgentUseCase,
    logger?: ILogger
  ) {
  ...
  async handle(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    ...
    try {
      ...
      const validatedBody = validateAgentCreateBody(event.body);
      ...
      if (this.structuredLogger && tenantId) {
        this.structuredLogger.logErrorWithContext(
          'Error in AgentCreateController',
          error as Error,
          {
            requestId,
            tenantId,
        ...
        this.logger.error('Error in AgentCreateController', error as Error, {
          requestId,
          path: event.path,
          method: event.httpMethod,
          tenantId,
