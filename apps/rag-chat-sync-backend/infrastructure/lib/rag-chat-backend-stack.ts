...
export class RagChatBackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const tavusApiKeyValue = process.env.TEST_API_KEY;
    if (!tavusApiKeyValue || tavusApiKeyValue.length < 20) {
      throw new Error('TEST_API_KEY must be set and at least 20 characters long for Tavus API key.');
    }
    ...
    const lambdaEnvironment = {
      ...
      TOP_K: '8',
      MAX_CITED_URLS: '3',
      TAVUS_API_KEY: tavusApiKeyValue,
    };
    ...
    // API Key and Usage Plan for Tavus integration
    const tavusApiKey = api.addApiKey('TavusApiKey', {
      apiKeyName: 'tavus-llm-key',
      description: 'API Key for Tavus custom LLM integration',
      value: tavusApiKeyValue,
    });

    const tavusUsagePlan = api.addUsagePlan('TavusUsagePlan', {
      name: 'Tavus LLM Usage Plan',
      description: 'Usage plan for Tavus API key access',
    });

    tavusUsagePlan.addApiKey(tavusApiKey);
    tavusUsagePlan.addApiStage({
    ...
    const lambdaFunctions = [knowledgeCreateLambda, knowledgeListLambda, agentCreateLambda, chatLambda];
    ...
    new cdk.CfnOutput(this, 'TavusApiKeyValue', {
      value: tavusApiKeyValue,
