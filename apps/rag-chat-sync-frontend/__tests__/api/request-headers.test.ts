...
describe('Property 22: API Request Headers', () => {
  ...
  describe('Authorization Header Tests', () => {
    it('Property: All API requests include Authorization header with JWT token', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(), // JWT token
          async (jwtToken) => {
    ...
    it('Property: Authorization header format is always "Bearer <token>"', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          async (token) => {
            // Create a fresh mock for this specific test run
            const localMockFetch = jest.fn().mockResolvedValue({
              ok: true,
              status: 200,
              headers: new Headers({ 'content-type': 'application/json' }),
              json: async () => ({ success: true }),
            ...
            try {
              ...
              try {
                ...
                // Ignore API errors, we're only testing headers
              }

              // Get the headers from the fetch call
              if (localMockFetch.mock.calls.length !== 1) {
                throw new Error(`Expected 1 call, got ${localMockFetch.mock.calls.length}`);
              }

              const callArgs = localMockFetch.mock.calls[0];
              const requestInit = callArgs[1] as RequestInit;
              const headers = requestInit.headers as Record<string, string>;
              ...
              if (authHeader !== expectedAuth) {
                throw new Error(`Expected "${expectedAuth}", got "${authHeader}"`);
              }
            } finally {
    ...
    it('Property: Authorization header persists across multiple requests', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.integer({ min: 1, max: 10 }), // number of requests (at least 1)
          async (token, numRequests) => {
            // Clear mock before each property run
            mockFetch.mockClear();
    ...
    it('Property: Token can be updated and subsequent requests use new token', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.uuid(),
  ...
  describe('Content-Type Header Tests for POST Requests', () => {
    it('Property: All POST requests include Content-Type: application/json header', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.record({
    ...
    it('Property: POST request bodies are valid JSON strings', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.record({
          ...
          async (token, agentData) => {
            // Pre-condition: ensure strings are not whitespace-only (would fail validation)
            fc.pre(agentData.name.trim().length > 0);
            fc.pre(agentData.description.trim().length > 0);
            fc.pre(agentData.knowledgeSpaceId.trim().length > 0);
            ...
            // Only check if fetch was actually called (validation passed)
    ...
    it('Property: All POST requests with body have both Authorization and Content-Type headers', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.record({
  ...
  describe('HTTP Method Tests', () => {
    it('Property: GET requests include Authorization but may not include Content-Type', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          async (token) => {
    ...
    it('Property: DELETE requests include Authorization header', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
  ...
  describe('Edge Cases', () => {
    it('Property: Empty token string results in no Authorization header', async () => {
      await fc.assert(
        fc.asyncProperty(
    ...
    it('Property: Special characters in token are preserved', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid().map(s =>
            s + '!@#$%^&*()_+-=[]{}|;:,.<>?'
    ...
    it('Property: Token with whitespace is preserved exactly', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.uuid(),
  ...
  describe('Header Invariants', () => {
    it('Property: Headers object is always defined for all requests', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.oneof(
    ...
    it('Property: Authorization header is never undefined when token is set', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          async (token) => {
    ...
    it('Property: Content-Type header is consistent across all POST requests', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
